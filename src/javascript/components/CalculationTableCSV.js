...
const CalculationTableCSV = ({
  ...
  const contentRows = [
    ...
    ["CIE XYZ tristimulus values [X]", ...chromaticity31.map(({ X }) => X)],
    ["CIE XYZ tristimulus values [Y]", ...chromaticity31.map(({ Y }) => Y)],
    ["CIE XYZ tristimulus values [Z]", ...chromaticity31.map(({ Z }) => Z)],
